...
// V1.1 => 23/05/2026 Changelog - Alteracoes
// Separate Low-Speed SW SPI and High-Speed SW SPI
// Separadas as funcoes de baixa e alta velocidade


/* DRIVER CONFIG */
#define SPI_CLK_HIGH    PORTC_OUT(3,1)      // SCLK PIN HIGH
#define SPI_CLK_LOW     PORTC_OUT(3,0)      // SCLK PIN LOW
#define SPI_SDO_HIGH    PORTC_OUT(4,1)      // MOSI PIN HIGH
#define SPI_SDO_LOW     PORTC_OUT(4,0)      // MOSI PIN LOW
#define SPI_SDI         PC2                 // MISO PIN READ

#define SPI_TEMP        Delay_Us(SPI_SPEED); // CONTROLE DE VELOCIDADE - SPEED

#define SPI_CS1_HIGH    PORTC_OUT(1,1)       // CHIP SELECT PIN HIGH
#define SPI_CS1_LOW     PORTC_OUT(1,0)       // CHIP SELECT PIN LOW
...
void SPI_CLOCK(){ SPI_CLK_HIGH; SPI_CLK_LOW; }
...
uint16_t SPI_SPEED=0; // 5 for +- 100Khz

void SPI_CLOCK_L(){ SPI_CLK_HIGH; SPI_TEMP; SPI_CLK_LOW; SPI_TEMP; }
...
// USE:
// SPI_DATAOUT(num ou bits,data); SPI_DATAOUT(8(byte),0xAD); SPI_DATAOUT(16(uint16_t),0xAF84);
